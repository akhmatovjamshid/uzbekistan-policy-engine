...
(`.github/workflows/data-regen.yml`; DFM entry to be added by the
subsequent PR in this slice). `attribution.timestamp` and
...
- **PR 3.** Overview page integration — switch nowcast source in
...
- **PR 4.** Nightly CI extension — add a `Rscript scripts/export_dfm.R`
  step to `.github/workflows/data-regen.yml` and include `dfm.json` in
  the nightly diff-check + commit step.
